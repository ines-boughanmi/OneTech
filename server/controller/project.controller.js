...
module.exports = {
  ...
  getProjectsByUser: async (req, res) => {
    try {
      let projects = [];
      let missions = [];
      const partitions = await db.Partition.findAll({
        where: { userId: req.params.id },
      });
      for (const partition of partitions) {
        const mission = await db.Mission.findOne({
          where: { id: partition.missionId, type: "normal" },
        });
  ...
  searchProjectsUser: async (req, res) => {
    try {
      let projects = [];
      let missions = [];
      const partitions = await db.Partition.findAll({
        ...
        if (mission) {
          const project = await db.Project.findAll({
            where: {
              ...
              project_title: { [Op.like]: `%${req.params.searchTerm}%` }
            },
          });
          projects.push(project);
        }
